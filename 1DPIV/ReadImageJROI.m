...
%           is preserved
% 20210907 Guanghan Meng: 'mnCoordinates' added for 'line' type ROI; 
% redundant values caused by subpixel ROI resolution in imageJ 1.5.2 
% onward have been removed
...
switch nTypeID
   ...
   case 3
      ...
      points = sROI.vnLinePoints;
      if abs(points(3) - points(1)) > abs(points(4) - points(2))
          flagV = ( points(3) - points(1) )/abs(points(3)-points(1));
          xCoor = points(1):flagV:points(3);
          yCoor = round( points(2) + ( points(4) - points(2) )/(points(3) - points(1))*(xCoor - points(1)));
      else
          flagV = ( points(4) - points(2) )/abs(points(4)-points(2));
          yCoor = points(2):flagV:points(4);
          xCoor = round( points(1) + ( points(3) - points(1) )/(points(4) - points(2))*(yCoor - points(2)));
      end
      Coordinates = zeros(length(xCoor),2);
      Coordinates(:,1) = xCoor;
      Coordinates(:,2) = yCoor;
      sROI.mnCoordinates = Coordinates;
      if (nROISubtype == 2)
...
% --- END of ReadImageJROI FUNCTION ---
   ...
   function [mnCoordinates] = read_coordinates
      ...
      % subpixel resolution in imageJ 1.5.2 onward leads to repetitive
      % integer values in the readout. Remove the redundant values 
      CoorIndx = ones(size(mnCoordinates,1),1);
      CoorIndx(2:end) = sum( abs(diff(mnCoordinates)), 2);
      mnCoordinates = mnCoordinates(CoorIndx>0, :);
