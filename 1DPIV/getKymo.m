...
for tifIndx = 1:1:numel(tifList)
    ...
    if sliceNum > 1
        ...
        if ~isempty(ROIlist)
            ...
            if max(data3D(:)) < 1.1
                dtypeSave = 'double';
            elseif max(data3D(:)) > 256
                dtypeSave = 'uint16';
            else
                dtypeSave = 'uint8';
            end
            % save morphology image - temporally averaged/summed 
            morphImg = mean(data3D, 3);
            if strcmp(dtypeSave,'uint8')
                if max(data3D(:))<15 
                    morphImg = sum(data3D,3);
                end
            end
            morphImg = (morphImg - min(morphImg(:)))./(max(morphImg(:)) - min(morphImg(:))); % normalize the morphology image; 
            imwrite(morphImg,fullfile(tifPath,[tifName(1:end-4),'_sum.tif']))
